...
namespace sones.Storage.Serializer
{
	...
	public sealed class SerializationWriter : BinaryWriter
	{
		...
		/// 1 or 16 bytes: Guid
		/// 
		/// 1 plus content: string, object[], byte[], char[], BitArray, myObjectStreamType, ArrayList
		/// 
		...
		/// <summary>
		/// Stores a myObjectStreamType object into the stream.
		/// Stored myNumberOfBytes: Depends on the length of the myObjectStreamType's name and whether the fullyQualified parameter is set.
		/// A null myObjectStreamType takes 1 byte.
		/// </summary>
		/// <param name="value">The myObjectStreamType to store.</param>
		...
		/// <summary>
		/// Stores a non-null myObjectStreamType object into the stream.
		/// Stored myNumberOfBytes: Depends on the length of the myObjectStreamType's name.
		/// If the type is a System type (mscorlib) then it is stored without assembly name information,
		/// otherwise the myObjectStreamType's AssemblyQualifiedName is used.
		/// </summary>
		/// <param name="value">The myObjectStreamType to store. Must not be null.</param>
		...
		/// </remarks>
		/// <typeparam name="K">The key myObjectStreamType.</typeparam>
		/// <typeparam name="V">The value myObjectStreamType.</typeparam>
		...
		/// </remarks>
		/// <typeparam name="T">The list myObjectStreamType.</typeparam>
		...
		/// The same token will always be used for the same object instance.
		/// 
		/// When recreateFromType is set to true, the object's myObjectStreamType will be stored and the object recreated using 
		...
		/// <summary>
		/// Checks whether instances of a myObjectStreamType can be created.
		/// </summary>
		/// <remarks>
		/// A Value myObjectStreamType only needs to implement IOwnedDataSerializable. 
		/// A Reference myObjectStreamType needs to implement IOwnedDataSerializableAndRecreatable and provide a default constructor.
		/// </remarks>
		/// <param name="type">The myObjectStreamType to check</param>
		/// <returns>true if the myObjectStreamType is recreatable; false otherwise.</returns>
		...
		#region Singleton myObjectStreamType Wrapper
		/// <summary>
		...
		#endregion Singleton myObjectStreamType Wrapper

		#region myObjectStreamType Usage (Debug mode only)
#if DEBUG
		...
		#endregion myObjectStreamType Usage (Debug mode only)
	...
	public sealed class SerializationReader : BinaryReader
	{
		...
		/// <summary>
		/// Returns a myObjectStreamType or null from the stream.
		/// 
		/// Throws an exception if the myObjectStreamType cannot be found.
		/// </summary>
		/// <returns>A myObjectStreamType instance.</returns>
		...
		/// Returns a myObjectStreamType or null from the stream.
		/// 
		/// Throws an exception if the myObjectStreamType cannot be found and throwOnError is true.
		/// </summary>
		/// <returns>A myObjectStreamType instance.</returns>
		...
		/// 
		/// </summary>
		/// <param name="elementType">The myObjectStreamType of the expected array elements. null will return a plain object[].</param>
		/// <returns>An object[] instance.</returns>
		...
		/// <summary>
		/// Returns a myObjectStreamType from the stream.
		/// 
		/// Throws an exception if the myObjectStreamType cannot be found.
		/// </summary>
		/// <returns>A myObjectStreamType instance.</returns>
		...
		/// <param name="defaultElementType">The myObjectStreamType of array element; null if to be read from stream.</param>
		/// <returns></returns>
	...
	internal enum SerializedType : byte
	{
		...
		TypeType,                  // Identifies a myObjectStreamType type 

		SingleInstanceType,        // Used internally to identify that a single instance object should be created
		// (by storing the myObjectStreamType and using Activator.GetInstance() at deserialization time)
		...
		// Placeholders to indicate number of myObjectStreamType Codes remaining
		ObjectLocatorType,
		Reserved23,
		Reserved22,
		Reserved21,
